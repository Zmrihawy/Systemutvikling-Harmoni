...
module.exports = class ServerDao extends Dao {
    ...
    getEventTickets(sql : number | string, callback: Function) {
        super.query(`SELECT * FROM ${CONSTANTS.TICKET_TABLE} WHERE ${CONSTANTS.EVENT_ID} = ?`, [sql], callback);
    }

    getPerformanceRiders(sql: string | number, callback: Function) {
    ...
    getUsersEvents(sql : {userId: string| number, active : string | number}, callback: Function) {
    ...
    /**CREATE*/
    createEvent(sql:{name : string, userId: string | number, startTime : string, endTime: string, location : string}, callback: Function) {
    ...
    createTicket(sql : {name : string, eventId : string | number, price : number | string, description: string, amount: string | number}, callback: Function) {
    ...
    createPerformance(sql: {artistId: string | number, eventId: string | number, startTime : string, endTime : string, contract: string}, callback: Function) {
    ...
    createRider(sql : {performanceId : string | number, name : string, amount : string |number}, callback: Function) {
        super.query(`INSERT INTO ${CONSTANTS.RIDER_TABLE} VALUES (?,?,?) `,
            [sql.performanceId, sql.name, sql.amount], callback);
    }

    /**DELETE*/
    deleteRider(sql: {performanceId: string |number, name : string}, callback: Function) {
    ...
    deleteEvent(sql: string, callback: Function) {
        super.query(`DELETE FROM ${CONSTANTS.EVENT_TABLE} WHERE ${CONSTANTS.EVENT_ID} = ?`, [sql], callback);
    }

    deleteTicket(sql: {name : string, eventId : string | number}, callback: Function) {
    ...
    deletePerformance(sql: string, callback: Function) {
        super.query(`DELETE FROM ${CONSTANTS.PERFORMANCE_TABLE} WHERE ${CONSTANTS.PERFORMANCE_ARTIST_ID} = ?`, [sql], callback);
    }

    /**UPDATE*/
    updateTicket(sql :{price : string | number, amount: string | number, description : string, name : string, eventId: string | number}, callback: Function) {
    ...
    updateRider(sql :{name : string, amount : string | number, performanceId : string | number, oldName : string}, callback: Function) {
    ...
    updatePerformance(sql : {startTime : string, endTime : string, contract : string, performanceId : string | number}, callback: Function) {
    ...
    updateEvent(sql: {eventName : string, hostId : string | number, active : string | number, location : string , startTime : string | number, endTime: string|number, eventId : string|number}, callback: Function) {
    ...
    updateContract(sql: {hostId : string|number, active: string|number, location : string, startTime : string |number, endTime : string|number, eventId : string|number}, callback: Function) {
        super.query(`UPDATE ${CONSTANTS.EVENT_TABLE} SET ${CONSTANTS.EVENT_NAME} = ?, ${CONSTANTS.EVENT_HOST_ID} = ?, ${CONSTANTS.EVENT_ACTIVE} = ?, ${CONSTANTS.EVENT_LOCATION} = ?, ${CONSTANTS.EVENT_START_TIME} = ?,
