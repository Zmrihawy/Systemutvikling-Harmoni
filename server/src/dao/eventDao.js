...
module.exports = class ServerDao extends Dao {
    ...
    getEvent(sql: string | number, callback: (status: number, data: *) => void) {
        ...
        // super.query(`SELECT * FROM ${CONSTANTS.EVENT_TABLE} e LEFT JOIN ${CONSTANTS.PERFORMANCE_TABLE} p ON e.${CONSTANTS.EVENT_ID} = p.${CONSTANTS.PERFORMANCE_EVENT_ID} WHERE e.${CONSTANTS.EVENT_ID} = ?`, [sql], callback);
    }
