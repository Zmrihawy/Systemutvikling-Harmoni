...
module.exports = class ServerDao extends Dao {
  ...
  createRiders(sql, callback) {
    super.query(`INSERT INTO ${CONSTANTS.RIDER_TABLE} (${CONSTANTS.RIDER_PERFORMANCE_ID},${RIDER_DESCRIPTION}) VALUES (?,?) `, [sql.performanceId, sql.description], callback);
  }
  /**DELETE*/
  ...
  updateTicket(sql, callback) {
    super.query(`UPDATE ${CONSTANTS.TICKET_TABLE} SET ${CONSTANTS.TICKET_PRICE} = ?, ${CONSTANTS.TICKET_AMOUNT} = ?, ${CONSTANTS.TICKET_DESCRIPTION} = ?  WHERE ${CONSTANTS.TICKET_NAME} = ? AND ${CONSTANTS.TICKET_EVENT_ID} = ? `[(sql.price, sql.description, sql.name, sql.eventId)], callback);
  }

  updateRider(sql, callback) {
    super.query(`UPDATE ${CONSTANTS.RIDER_TABLE} SET ${CONSTANTS.RIDER_NAME} = ?, ${CONSTANTS.RIDER_AMOUNT} = ? WHERE ${CONSTANTS.RIDER_PERFORMANCE_ID} and ${CONSTANTS.RIDER_NAME} = ?`, [sql.name, sql.amount, sql.performanceId, sql.oldName], callback);
  }

  updatePerformance(sql, callback) {
    super.query(`UPDATE ${CONSTANTS.PERFORMANCE_TABLE} SET ${CONSTANTS.PERFORMANCE_START_TIME} = ?, ${CONSTANTS.PERFORMANCE_END_TIME} = ?, ${CONSTANTS.PERFORMANCE_CONTRACT} = ? WHERE ${CONSTANTS.PERFORMANCE_ID} = ?`, [sql.startTime, sql.endTime, sql.contract, sql.performanceId], callback);
  }
