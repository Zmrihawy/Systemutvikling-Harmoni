...
import { faUser, faFile, faLocationArrow, faCalendarAlt } from '@fortawesome/free-solid-svg-icons';

const eventInfo = props => {
    return (
        <div className={classes.container}>
            <h1 className={classes.title}>{props.title}</h1>
            <button className={classes.button__edit}>Rediger arrangement</button>
            ...
            <div className={classes.info}>
                <div className={`${classes.location} ${classes.info__item}`}>
                    ...
                    <p className={classes.info__description}>{props.location}</p>
                    <FontAwesomeIcon className={classes.info__icon} icon={faCalendarAlt} />
                </div>
                <div className={`${classes.date} ${classes.info__item}`}>
                    ...
                    <FontAwesomeIcon className={classes.info__icon} icon={faLocationArrow} />
                </div>
                <div className={`${classes.host} ${classes.info__item}`}>
                    ...
                    <FontAwesomeIcon className={classes.info__icon} icon={faUser} />
                </div>
                <div className={`${classes.ticketAmount} ${classes.info__item}`}>
                    <h2 className={classes.info__title}>Antall billetter</h2>
                    <p className={classes.info__description}>{props.ticketAmount}</p>
                    <FontAwesomeIcon className={classes.info__icon} icon={faFile} />
                </div>
            </div>

            <div className={classes.map}>
                <Map longitude={props.longitude} latitude={props.latitude} location={props.location} />
            </div>
                ...
                <div className={classes.artist__list}>
                    {props.artists.map((artist, i) => (
                        <div className={classes.artist__wrapper} key={i}>
                            ...
                            <h3>{artist.name}</h3>
                            <button className={classes.button__contract}>Kontrakt</button>
                ...
                <div className={classes.staff__list}>
                    {props.staff.map((staff, i) => (
                        <div className={classes.staff__wrapper} key={i}>
                            <h2 className={classes.staff__name}>{staff.name}</h2>
                            <p className={classes.staff_proffesion}>{staff.proffesion}</p>
                        </div>
                    ))}
                </div>
            </div>
